...
- Optional typed preparation: `npm_build`, which runs exactly `npm run build`; arbitrary build commands remain forbidden.
- Trusted workflow runtime: Node.js 24.x from pinned `actions/setup-node`.
...
- Target acquisition, install hooks, optional build, and runtime execute as a dedicated unprivileged OS user.
...
Use `"prepare_profile": "npm_build"` only when direct repository evidence shows that the expected JavaScript entrypoint is generated by the package's standard `build` script. Otherwise omit the field or use `"prepare_profile": "NONE"`. `build_command` and other free-form command fields are rejected.
...
The result records request scope, environment/isolation assumptions, exact commit, Node/npm/lockfile provenance, preparation profile/status, collectors and limitations, bounded observations, stop-condition state, and errors. It never emits a security verdict.

Filesystem observations are phase-separated: dependency installation is `INSTALL`, optional `npm_build` preparation is `PREPARE`, and target runtime changes are `EXERCISE`. Each list remains bounded and records its limitation.

## Controlled fixtures

CI validates the harness against this repository's exact commit using safe, build-required, noisy, and failure fixtures.

The fixture job verifies the persistent MCP initialize lifecycle, typed build preparation, receipt of `notifications/initialized`, process cleanup, trusted-checkout isolation, phase-separated filesystem observations, process/network traces, and timeout behavior before changes are accepted on `main`.
