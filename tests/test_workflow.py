...
class WorkflowTests(unittest.TestCase):
    ...
    def test_target_cannot_traverse_trusted_workspace(self):
        text = Path('.github/workflows/runtime-review.yml').read_text(encoding='utf-8')
        self.assertIn('sudo chmod 700 /home/runner', text)
        self.assertIn('sudo chmod 700 "$GITHUB_WORKSPACE"', text)

    def test_ci_runs_controlled_runtime_fixtures(self):
        text = Path('.github/workflows/ci.yml').read_text(encoding='utf-8')
        self.assertIn('runtime-fixtures:', text)
        self.assertIn('tests/runtime_fixture_check.py', text)
